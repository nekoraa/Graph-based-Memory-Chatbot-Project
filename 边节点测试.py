...
from 图保存测试 import 显示交互式图
from 记忆体 import 记忆图操作函数
from 记忆体.记忆图操作函数 import 检查或加载记忆图, 扩散传播记忆算法, 提取问题
...
#
# 词嵌入函数.添加节点总("蕾米莉亚", 图)
# 词嵌入函数.添加节点总("东方project", 图)
# 词嵌入函数.添加节点总("芙兰朵露", 图)
# 词嵌入函数.添加节点总("幻想乡", 图)
# 词嵌入函数.添加节点总("红魔馆", 图)
# 词嵌入函数.添加节点总("平常并不太易怒，但精神有些问题。", 图)
#
# 词嵌入函数.添加边("蕾米莉亚", "东方project", 图, '蕾米莉亚属于东方project这个游戏')
# 词嵌入函数.添加边("芙兰朵露", "东方project", 图, '芙兰朵露属于东方project这个游戏')
# 词嵌入函数.添加边("幻想乡", "东方project", 图, '幻想乡是东方project这个游戏里的世界名称')
# 词嵌入函数.添加边("红魔馆", "东方project", 图, '红魔馆是东方project这个游戏世界里的一个地区')
# 词嵌入函数.添加边("蕾米莉亚", "芙兰朵露", 图, '蕾米莉亚是芙兰朵露的姐姐')
# 词嵌入函数.添加边("蕾米莉亚", "红魔馆", 图, '蕾米莉亚住在红魔馆')
# 词嵌入函数.添加边("芙兰朵露", "红魔馆", 图, '芙兰朵露住在红魔馆')
# 词嵌入函数.添加边("平常并不太易怒，但精神有些问题。", "芙兰朵露", 图, '芙兰朵露的性格')
...
# }"""
#
# j数据 = 词嵌入函数.字符串转换为json(数据)
# 节点, 边 = 词嵌入函数.转换json为数组(j数据)
# 图 = 词嵌入函数.创建图关系(节点, 边)

# 输入1 = """bilibili，音译哔哩哔哩，是总部位于中华人民共和国上海市的一个以ACG相关内容起家的弹幕视频分享网站，简称及通称B站。此外，bilibili的前身为视频分享网站Mikufans，并由网友“⑨bishi”于2009年6月26日创建。Mikufans建站的初衷是为用户提供一个稳定的弹幕视频分享网站，其后于2010年1月24日改为“bilibili”。"""
# 输入2 = """bilibili，音譯嗶哩嗶哩，是總部位於中華人民共和國上海市的一個以ACG相關內容起家的彈幕影片分享網站，簡稱及通稱B站。此外，bilibili的前身為影片分享網站Mikufans，並由網友「⑨bishi」（徐逸）[11]於2009年6月26日建立。Mikufans建站的初衷是為使用者提供一個穩定的彈幕影片分享網站，其後於2010年1月24日改為「bilibili」。"""
# 图1 = 记忆图操作函数.输入信息生成知识图谱(输入1)
# print("-------------------------------------------")
# 图2 = 记忆图操作函数.输入信息生成知识图谱(输入2)
#
# 图3 = 记忆图操作函数.合成图(图1, 图2)
# print(记忆图操作函数.遍历边(图3))
# print(记忆图操作函数.遍历节点(图3))
# nx.draw(图3, with_labels=True, node_color='lightblue', node_size=2000, font_size=16, font_weight='bold',
#         edge_color='gray')
# # 显示图
# plt.show()
#
# 记忆图 = 检查或加载记忆图()
#
# 图数组 = 记忆图操作函数.检索图函数(记忆图, "狗", 1, 0.5, 100,20)
#
#
# nx.draw(图数组[0], with_labels=True, node_color='lightblue', node_size=2000, font_size=16, font_weight='bold',
#         edge_color='gray')
# # 显示图
# plt.show()


# 输出 = 提取问题(记忆图, "用户家的狗叫什么?")
# print(输出)
#
# print("-------------------------------------------")
# print(记忆图操作函数.提取整理记忆(图数组))
# print("-------------------------------------------")

输入 = """东方Project（日文：東方Project（とうほうプロジェクト），英文：Touhou Project，简称：东方）
...
第一部官方作品东方灵异传于1996年发布，此后，ZUN始终以一定频率持续发布新作品，至今为止已有弹幕射击游戏、弹幕格斗游戏、漫画、小说、设定集、独立音乐CD等等形式多样的作品。
...
由“上海爱丽丝幻乐团”制作，并且给予小数点系列编号的游戏被称为小数作（此处不包含和黄昏合作的作品）。
到目前为止，小数作共有6部。
...
情节与故事设定的共通内容有：
博丽灵梦、雾雨魔理沙是系列的共通主角，绝大多数官方作品的故事情节都围绕此二人展开。
博丽灵梦是系列的第一主角，围绕灵梦的角色形象产生的一系列符号印象，如阴阳玉、红色大缎带、独特的巫女服饰等等，已然深入人心，成为了足以代表东方Project系列的经典元素。
...
妖怪、神明等等民俗传说和神话中的形象是东方Project的主要描绘对象。
除少数早期设计之外，几乎所有的东方角色的设计原型和灵感来源都来自于民俗传说或神话。

创作主题与设计思想的共通内容有：
从《东方红魔乡》开始，ZUN就非常注重于在作品中体现和洋折衷这一创作主题。
...
官方作品的影响
吉尼斯世界纪录认证“最多作品的同人射击游戏系列”（2010年10月1日）3
日本独立游戏界专题纪录片Branching Paths
东方的原作者
ZUN
...
以下列出部分参与过官方作品的作者：
担任过官方漫画作画的画师：松仓眠、比良坂真琴、秋★枝、荒田俊平、あずまあや、春河萌、水炊き、银木犀、秋巻ゆう

在官方出版物中负责插图的画师：哑采弦二、TOKIAME、綾見ちは、高阶@圣人、匡吉、ゆきうさぎ、六羽田共、相生青唯、あらたとしひら、ALISON、水中花火、叶庭、火鸟……
除黄昏作之外，在官方游戏中参与美术的画师：Alphes、Ki-san、海原海豚
东方二次创作集合
...
2004年2月，在当时的爱好者协助下，ZUN发表了第一份东方二次创作规约，此后，规约内容进行过多次修改与补充。
2004年4月，第一届博丽神社例大祭举办。例大祭是世界最大规模的东方Only同人贩卖会，也一度是日本最大规模的作品主题Only同人贩卖会，最多的时候摊位数达到5000以上。
...
二次创作社群的影响
同人方面
东方Project在全世界最大的同人展会Comic Market上拥有独立分类编码。
...
除此之外，东方Project还拥有复数的授权二次创作商业手游，如东方大炮弹、东方LostWord、东方弹幕神乐等。"""

信息图 = 记忆图操作函数.长信息记忆图构建(输入)
显示交互式图(信息图)
